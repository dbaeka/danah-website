...
function read_datas(){
	$myfile = fopen("cn_case.txt", "r") or die("Unable to open file!");
		$line = fread($myfile,filesize("cn_case.txt"));
		$datas = explode("\n", $line);
		$sections = [];
		$questions = [];
		$id = 0;
		...
		fclose($myfile);	
		return ["cn_sections"=> $sections, "cn_questions"=>$questions];
}
...
if ($sections == null){
	$results = read_datas();
	$_SESSION["cn_sections"] = $sections = $results["cn_sections"];
	$_SESSION["cn_questions"] = $questions = $results["cn_questions"];
}
$token = $_GET['token'];
if ($token == null){
						...
						<form class="w-100" action="submitTest.php" method="post">
							...
							<div class="form-group " style="background-color: rgba(220,220,220,0.2); padding: 10px; border-radius: 20px;">
						  	...
						  	<div class="float-right">
								...
								<?php if ($page!=0){?>
									<a href="cn_test.php?page=<?=$page-1?>&token=<?=$token?>" style="margin: 10px;">
										<button type="button" class="btn btn-primary btn-lg" style="background-color: #fd79a8; border: solid 1px #fd79a8;">上一页</button>	
									</a>
								<?php }?>
						  		

						  		<input type="submit" value="下一页" class="btn btn-primary btn-lg" style="background-color: #fd79a8; border: solid 1px #fd79a8;">
